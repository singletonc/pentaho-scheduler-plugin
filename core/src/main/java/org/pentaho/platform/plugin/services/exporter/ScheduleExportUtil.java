...
public class ScheduleExportUtil implements IExportHelper {
  ...
  protected void exportSchedules() throws ExportException {
    log.info("*************************** Start [ Export schedule(s) ] *******************************");
    int jobListSize = 0;
    ...
    try {
      ...
      log.info("Found [ " + jobListSize + " ] schedule(s)");
      for ( Job job : jobs ) {
        ...
        try {
          ...
          log.warn("Error adding schedule [ " + job.getJobName() + " ] cause [ " + e.getMessage() + " ]");
          log.debug("Error adding schedule [ " + job.getJobName() + " ] cause [ " + e.getMessage() + " ]", e );
        }
      }
    } catch ( SchedulerException e ) {
      throw new ExportException( Messages.getInstance().getString( "PentahoPlatformExporter.ERROR_EXPORTING_JOBS" ), e );
    } finally {
      log.info("Successfully exported [ " + successfulJobExportCount + " ]" +
              " out of [ " +  jobListSize + " ] Schedule(s)");
      log.info("*********************************  [ End: Export Schedule(s) ] *******************************");
    }
  }
